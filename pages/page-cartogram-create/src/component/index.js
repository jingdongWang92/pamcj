...
class CartogramCreate extends React.Component {

  static propTypes = {
    handleSubmit: PropTypes.func.isRequired,
    createCartogram: PropTypes.func.isRequired,
  ...
  render() {
    ...
    return (
      <Page>
        <Card bordered={false} title={<CardTitle text="创建地图" />} extra={<CardExtra isSubmitting={isSubmitting} />}>
          <Form id={constants.MODULE_NAME} onSubmit={handleSubmit(createCartogram)}>
            <Row gutter={32}>
              <Col span={12}>
                <Field
                ...
                >
                  {organizations.map(organization => (
                    <ReduxFormSelect.Option value={organization.id} key={organization.id}>{organization.name}</ReduxFormSelect.Option>
                  ))}
                </Field>
              </Col>
            </Row>
...
export default reduxForm({ form: constants.MODULE_NAME })(CartogramCreate);
